...
function ModificarUsuarioSucursal (datos){
    $.ajax({
        url:"ModificarUsuarioSucursal",
        data: datos,
        type:"POST",
        beforeSend: function () {
            $.blockUI({
...
$("#Limpiar").click(function(){
    ...
    $("#sucs_disp").find("option:selected").prop("selected", false);
    $('#sucs_asignar_r').find('option').each(function () {
        $(this).clone().appendTo('#sucs_asignar');
    });
    $("#sucs_asignar").find("option:selected").prop("selected", false);
});
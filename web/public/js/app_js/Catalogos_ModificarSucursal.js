$(document).ready(function(){
    ...
    $('#frmSucursal').find('input, button, select').prop('disabled', true);
...
$("#Editar").click(function(){
    ...
    var docs = $("#documentos").val();
    if (docs === "") {
        $('#frmSucursal').find('input, button, select').prop('disabled', false);
        $("#apertura").prop('readonly', true);
        ponerFoco("sucursal");
    }
    else {
        $("#nombre, #Guardar").prop('disabled', false);
        ponerFoco("nombre");
    }
...
$("#Guardar").click(function() {
    ...
    $('#frmSucursal').find('input, select').prop('disabled', false);
    var datos = JSON.stringify($("#frmSucursal").serializeObject());
    if ($("#documentos").val() !== "") {
        $('#frmSucursal').find('input, select').prop('disabled', true);
        $("#nombre, #Guardar").prop('disabled', false);
    }
    //datos = cambiarFormatoFecha(datos);
    var msj = validarInfo(datos);
    if (msj == "")
        ModificarSucursal("ModificarSucursal", datos);
    else
...
function ModificarSucursal(url, datos)
{
    $.ajax({
        url:url,
        type:'POST',
